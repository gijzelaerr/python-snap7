"""
Snap7 client using ctypes to interface with the native Snap7 C library.
"""
...
from snap7.error import error_wrap, check_error
...
class ClibClient(Client):
    """
    A snap7 client using ctypes to interface with the native Snap7 C library.

    This is the traditional implementation that requires the Snap7 C library to be installed.

    Examples:
        >>> import snap7
        >>> client = snap7.client.Client()  # or ClibClient() directly
    ...
    def __init__(self, lib_location: Optional[str] = None, **kwargs):
        """Creates a new `ClibClient` instance.
        ...
        Examples:
            >>> from snap7.clib_client import ClibClient
            >>> client = ClibClient()  # If the `snap7.dll` file is in the path location
            >>> client2 = ClibClient(lib_location="/path/to/snap7.dll")  # If the dll is in another location
        """

        self._lib: Snap7CliProtocol = load_library(lib_location)
        self.create()

    def __enter__(self) -> "ClibClient":
        return self
    ...
    def connect(self, address: str, rack: int, slot: int, tcp_port: int = 102) -> "ClibClient":
