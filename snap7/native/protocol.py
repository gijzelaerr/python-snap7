...
from .errors import S7ProtocolError
...
class S7Protocol:
    ...
    def build_read_request(self, area: S7Area, db_number: int, start: int, 
                          word_len: S7WordLen, count: int) -> bytes:
        """
    ...
    def build_write_request(self, area: S7Area, db_number: int, start: int,
                           word_len: S7WordLen, data: bytes) -> bytes:
        """
    ...
    def build_setup_communication_request(self, max_amq_caller: int = 1, 
                                        max_amq_callee: int = 1,
                                        pdu_length: int = 480) -> bytes:
        """
    ...
    def extract_read_data(self, response: Dict[str, Any], word_len: S7WordLen, 
                         count: int) -> List[Any]:
        """
        ...
        # Decode data according to word length
        return S7DataTypes.decode_s7_data(raw_data, word_len, count)
