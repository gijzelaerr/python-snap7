...
class TestPureMainloop:
    ...
    def test_get_server_function(self):
        """Test the get_server function."""
        # Test default (ctypes) server
        server1 = snap7.get_server(pure_python=False)
        assert server1.__class__.__name__ == "Server"
        
        # Test pure Python server
        server2 = snap7.get_server(pure_python=True)
        assert server2.__class__.__name__ == "Server"
        
        # Both should have the same API
        common_methods = ['start', 'stop', 'register_area', 'get_status']
        for method in common_methods:
            assert hasattr(server1, method)
            assert hasattr(server2, method)
            assert callable(getattr(server1, method))
            assert callable(getattr(server2, method))